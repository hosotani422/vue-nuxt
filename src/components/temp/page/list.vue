...
<div class="absolute z-[10] top-0 bottom-0 left-0 w-[200%] theme-mask-color
  ...
  <div class="absolute z-[1] top-0 bottom-0 left-0 w-[43%] flex flex-col theme-grad-color theme-shadow-normal">
    <div class="relative z-[9] flex-auto flex items-center p-3 gap-3 theme-grad-color theme-shadow-normal">
      ...
      <p class="flex-even text-xl line-clamp-1">{{title}}</p>
      <ItemIconLeft class="flex-auto" @click="$emit(`routerBack`)" />
    </div>
    <ul ref="wrap" class="flex-even p-3 overflow-auto select-none">
      <transition-group>
        <li class="overflow-hidden relative h-16 flex items-center p-3 gap-3
          ...
          <p class="flex-even line-clamp-1" :class="classLimit(listId)">{{stateUnit(listId).title}}</p>
          <p class="flex-auto" :class="classLimit(listId)">{{textCount(listId)}}</p>
          <transition>
            <div class="absolute right-3 flex gap-3 slide-right theme-back-color" v-show="classItem(listId).edit">
              <ItemIconClone v-if="listId !== trashId"
                :class="classLimit(listId)" @click="$emit(`copyItem`, {event: $event, listId})" />
              <ItemIconTrash v-if="listId !== getListId() && listId !== trashId"
