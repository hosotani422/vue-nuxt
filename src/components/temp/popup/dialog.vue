...
<template>
<BasePopup data-test="DialogPage" :open="state.open">
  ...
  <div class="flex flex-col gap-3 overflow-auto">
    ...
    <ItemInputTextbox data-test="DialogTitle" class="border-solid border-b-[0.1rem] border-b-theme-fine"
    ...
    <ItemInputCheck data-test="DialogCheck" :key="`check${checkId}`" v-for="checkId of state.check.sort" v-if="state.mode === `check`"
    ...
    <ItemInputRadio data-test="DialogRadio" :value="radioId" v-if="state.mode === `radio`" v-for="radioId of state.radio.sort"
      :key="`radio${radioId}`" v-model="state.radio.select">{{state.radio.data[radioId]!.title}}</ItemInputRadio>
  </div>
  <div class="flex items-center justify-end gap-4">
    <ItemInputButton data-test="DialogCancel" class="flex-auto text-theme-fine"
      @click="state.callback.cancel!()">{{state.cancel}}</ItemInputButton>
    <ItemInputButton data-test="DialogOk" class="flex-auto text-theme-warn" v-if="state.mode !== `alert`"
