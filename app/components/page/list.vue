...
<template>
  ...
  >
    ...
    />
    <div class="theme-color-grad theme-shadow-outer absolute inset-y-0 left-0 z-[1] flex w-[43%] flex-col">
      <header class="theme-color-grad theme-shadow-outer relative z-[9] flex flex-initial items-center gap-3 p-3">
        <IconPlus class="flex-initial" @click="list.handle.entryItem()" />
        <h1 class="line-clamp-1 flex-1 text-xl">{{ app.refer.constant.app.name }}</h1>
        <IconArrow class="flex-initial rotate-180" @click="app.handle.routerBack()" />
      </header>
      <main class="flex-1 select-none overflow-auto p-3">
        <ul>
          <transition-group>
            ...
            >
              <component :is="list.render.typeIcon({ listId })" class="flex-initial" />
