...
const useStore = defineStore(`list`, () => {
  ...
  const render = reactive({
    classStatus: computed(() => (arg: { listId: string }): { [K in `select` | `edit` | `hide`]: boolean } => {
      return {
        select: app.render.listId() === arg.listId,
        edit: state.status[arg.listId] === `edit`,
        hide: state.status[arg.listId] === `hide`,
      };
    }),
    classLimit: computed(() => (arg: { listId: string }): { [K in `text-theme-care` | `text-theme-warn`]: boolean } => {
  ...
  const handle = {
    init: async (): Promise<void> => {
      state.data = process.client ? (await JSON.parse(localStorage.getItem(`list`)!)) || refer.init : refer.init;
      watch(
        () => state.data,
        (data) => {
          process.client && localStorage.setItem(`list`, JSON.stringify(data));
        },
        { deep: true },
      );
    },
