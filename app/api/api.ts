...
export const readList = async (): Promise<typeof list.state.data> => {
  if (conf.state.data.save === `rest`) {
    ...
    return JSON.parse(localStorage.getItem(`list`)!) || list.refer.init;
  }
  return list.refer.init;
};
...
export const readMain = async (): Promise<typeof main.state.data> => {
  if (conf.state.data.save === `rest`) {
    ...
    return JSON.parse(localStorage.getItem(`main`)!) || main.refer.init;
  }
  return main.refer.init;
};
...
export const readSub = async (): Promise<typeof sub.state.data> => {
  if (conf.state.data.save === `rest`) {
    ...
    return JSON.parse(localStorage.getItem(`sub`)!) || sub.refer.init;
  }
  return sub.refer.init;
};
...
export const readConf = async (): Promise<typeof conf.state.data> => {
  if (conf.state.data.save === `rest`) {
    ...
    return JSON.parse(localStorage.getItem(`conf`)!) || conf.refer.init;
  }
  return conf.refer.init;
};
...
export const readRoute = async (): Promise<string> => {
  if (conf.state.data.save === `rest`) {
    ...
    return localStorage.getItem(`route`) || app.refer.constant.id.inbox;
  }
  return app.refer.constant.id.inbox;
};
