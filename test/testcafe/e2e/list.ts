...
test(`route`, async(t: any) => {
  await t
    .click(Page.getById(`ListLeft`))
    .expect(Page.getById(`ListRoot`).count).eql(0);
});
...
test(`clone`, async(t: any) => {
  ...
  await t
    .click(Page.getById(`ListClone`))
    .expect(Page.getById(`ListItem`).count).eql(4);
});
test(`edit`, async(t: any) => {
  await t
    ...
    .expect(Page.getById(`ListTask`).nth(0).innerText).eql(`list0`);
});
test(`delete`, async(t: any) => {
  ...
  await t
    ...
    .expect(Page.getById(`ListItem`).count).eql(2)
    .click(Page.getById(`NoticeBack`))
    .expect(Page.getById(`ListItem`).count).eql(3);
});
test(`current`, async(t: any) => {
  await Page.longClick(Page.getById(`ListItem`).nth(0));
  await t
    .expect(Page.getById(`ListClone`).count).eql(1)
    .expect(Page.getById(`ListTrash`).count).eql(0);
});
test(`trash`, async(t: any) => {
  await Page.longClick(Page.getById(`ListItem`).nth(-1));
  await t
    .expect(Page.getById(`ListClone`).count).eql(0)
    .expect(Page.getById(`ListTrash`).count).eql(0);
});
